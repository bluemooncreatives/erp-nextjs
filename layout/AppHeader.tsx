...
// ---------------------------------------------------------------------------
// Header - the TailAdmin header, rewired to the ERP's controls:
//
...
function BranchSelect({
  ...
  const className =
    'h-11 rounded-lg border border-gray-200 bg-transparent px-3 text-sm text-gray-700 shadow-theme-xs focus:border-brand-300 focus:outline-hidden dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 hidden sm:block';

  if (!editable) {
    return (
      <span className={`${className} flex items-center`}>
        {branches.find((b) => b.id === current)?.name ?? 'Login Again'}
      </span>
    );
  }
...
function LanguageSelect({
  ...
  return (
    <select
      ...
      className="hidden h-11 rounded-lg border border-gray-200 bg-transparent px-3 text-sm text-gray-700 shadow-theme-xs focus:border-brand-300 focus:outline-hidden dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 sm:block"
    >
      {languages.map((l) => (
