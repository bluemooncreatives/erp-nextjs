...
export function CnfList({ rows, total, page, perPage, search, canCreate, canEdit, canDelete }: {
  ...
  return <ReferenceCrud title="CNF Agents" singular="CNF Agent" rows={rows.map((row) => ({ id: row.id, name: row.name ?? '', status: row.status, extra: [row.address, row.email, row.phone] }))}
    ...
    extraFields={[
      { name: 'email', label: 'Email', kind: 'email', values: Object.fromEntries(rows.map((row) => [String(row.id), row.email ?? ''])) },
      { name: 'phone', label: 'Phone', values: Object.fromEntries(rows.map((row) => [String(row.id), row.phone ?? ''])) },
      { name: 'address', label: 'Address', kind: 'textarea', values: Object.fromEntries(rows.map((row) => [String(row.id), row.address ?? ''])) },
    ]} />;
}
