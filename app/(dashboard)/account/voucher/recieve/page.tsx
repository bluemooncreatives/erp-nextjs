...
export default async function ReceiptVouchersPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <>
    <PageHeader title="Receipt Vouchers" breadcrumb={[{ label: 'Accounts' }, { label: 'Receipt Vouchers' }]}
      actions={canCreate ? <Link href={ROUTES['voucher_recieve.create']} className="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white">Add Receipt</Link> : null} />
    <Card title={`Receipts (${total})`} bodyClassName="">
      <DataTable columns={[{ label: 'Voucher' }, { label: 'Date' }, { label: 'Type' }, { label: 'Accounts' }, { label: 'Amount' }, { label: 'Approval' }, { label: 'Actions' }]} isEmpty={!rows.length} empty="No receipt vouchers found.">
        {decorated.map((voucher) => <Tr key={voucher.id}>
          <Td>{canView ? <Link className="text-brand-500" href={route('vouchers.show', { id: voucher.id })}>{voucher.txId ?? voucher.id}</Link> : voucher.txId ?? voucher.id}</Td>
          <Td>{voucher.dateLabel}</Td><Td>{voucher.voucherType}</Td>
          <Td>{voucher.legs.map((leg, index) => <span className="block text-theme-xs" key={index}>{leg.type}: {leg.accountName}</span>)}</Td>
          ...
          <Td><div className="flex items-center gap-3">
            {canEdit ? <Link className="text-brand-500" href={route('voucher_recieve.edit', { id: voucher.id })}>Edit</Link> : null}
