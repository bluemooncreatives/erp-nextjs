...
export default async function TransferDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="space-y-6">
      <Card title="Transfer details"><dl className="grid gap-4 text-sm text-gray-700 dark:text-gray-300 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label}><dt className="font-semibold">{label}</dt><dd>{value}</dd></div>)}</dl></Card>
      <Card title="Products" bodyClassName="">
        ...
        <div className="space-y-2 p-6 text-right text-sm text-gray-700 dark:text-gray-300"><p>Total products: {items.reduce((sum, item) => sum + item.quantity, 0)}</p><p>Total: {await singlePrice(items.reduce((sum, item) => sum + item.subTotal, 0))}</p></div>
      </Card>
      <Card title="Notes"><p className="whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300">{transfer.notes ?? '-'}</p></Card>
      {documents.length > 0 && <Card title="Documents"><ul className="space-y-2">{documents.map((document, index) => <li key={`${document}-${index}`}><a className="text-sm text-brand-500 underline" href={assetUrl(document) ?? '#'} download>Document {index + 1}</a></li>)}</ul></Card>}
    </div>
  </>;
}
